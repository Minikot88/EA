...
def _validate_payload(
    *,
    ...
    try:
        ...
        raise ValueError("capsule_invalid: local voice bootstrap cannot be read") from error
    if (
        not isinstance(sidecar, dict)
        or sidecar.get("schemaVersion") != 2
        or sidecar.get("policySha256") != manifest["policy_sha256"]
        or not isinstance(sidecar.get("policyVersion"), str)
        or not all(
