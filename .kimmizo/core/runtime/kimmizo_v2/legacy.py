...
def resolve_role_first(
    *,
    ...
    elif _needs_team(task, route):
        if not available:
            decision = {
                "status": "scoped",
                "execution_mode": "solo-reviewed" if review_required else "solo",
                "reason": "backend_unavailable_auto_fallback",
            }
        else:
            add_team_assignments()
            decision = {"status": "scoped", "execution_mode": "team", "reason": "auto_selected_team"}
