...
//--------------------------------------------------
const datetime EA_EXPIRE_AT = D'2026.08.15 23:59:59';
const string License_Contact = "สนใจ ea ติดต่อ tiktok : kontuengraph";
//--------------------------------------------------
...
void Information()
  {
   ...
   SetText("4",  "EA DASHBOARD Susanoo",                                 10,Y+40,  12,"Arial Black",clrAqua);
...
bool IsAccountAllowed()
  {
   // อนุญาต Strategy Tester สำหรับ Backtest
   if(MQLInfoInteger(MQL_TESTER))
      return(true);

   long trade_mode = AccountInfoInteger(ACCOUNT_TRADE_MODE);

   // อนุญาตเฉพาะบัญชี DEMO เท่านั้น
...
bool LicenseCheck(bool show_alert)
  {
   if(!IsAccountAllowed())
     {
      AutoRun = false;
      string msg = "EA นี้อนุญาตให้ใช้งานเฉพาะบัญชี DEMO เท่านั้น\n" + License_Contact;
      Comment(msg);
      Print(msg);
