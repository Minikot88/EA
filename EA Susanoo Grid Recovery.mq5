...
{
   int X = 8;              // ชิดขวา
   int Y = 18;             // เริ่มจากด้านบน
   int fsTitle = 10;
   int fs = 8;             // เล็กลง เหมาะกับหลายกราฟ
   int step = 16;          // ระยะห่างบรรทัด

   color normalCol   = clrWhite;
   color headCol     = clrAqua;
   ...
   SetText("4",  "╔════════ EA DASHBOARD ════════╗", X, Y + step*row++, fs,      "Consolas", headCol);
   SetText("5",  "║ " + statusText + " | " + _Symbol,      X, Y + step*row++, fsTitle, "Consolas", statusCol);
   SetText("6",  "╠══════════════════════════════╣",        X, Y + step*row++, fs,      "Consolas", headCol);

   SetText("7",  "║ Magic   " + IntegerToString(MagicNumber),   X, Y + step*row++, fs, "Consolas", normalCol);
   SetText("8",  "║ Entry   " + FirstEntryModeName(),            X, Y + step*row++, fs, "Consolas", normalCol);
   SetText("9",  "║ Mode    " + ModeTradeName(),                 X, Y + step*row++, fs, "Consolas", normalCol);
   SetText("10", "║ Time    " + timeText,                        X, Y + step*row++, fs, "Consolas", timeCol);

   SetText("11", "╠════════ BUY ═════════════════╣",             X, Y + step*row++, fs, "Consolas", clrDeepSkyBlue);
   SetText("12", "║ Orders  " + IntegerToString(CountB),         X, Y + step*row++, fs, "Consolas", normalCol);
   SetText("13", "║ Lot     " + DoubleToString(B_Lot,2),         X, Y + step*row++, fs, "Consolas", normalCol);
   SetText("14", "║ Avg     " + ShowPriceOrDash(B_Average),      X, Y + step*row++, fs, "Consolas", normalCol);
   SetText("15", "║ Profit  " + MoneyText(SumProfitBuy),         X, Y + step*row++, fs, "Consolas", buyCol);

   SetText("16", "╠════════ SELL ════════════════╣",             X, Y + step*row++, fs, "Consolas", clrTomato);
   SetText("17", "║ Orders  " + IntegerToString(CountS),         X, Y + step*row++, fs, "Consolas", normalCol);
   SetText("18", "║ Lot     " + DoubleToString(S_Lot,2),         X, Y + step*row++, fs, "Consolas", normalCol);
   SetText("19", "║ Avg     " + ShowPriceOrDash(S_Average),      X, Y + step*row++, fs, "Consolas", normalCol);
   SetText("20", "║ Profit  " + MoneyText(SumProfitSell),        X, Y + step*row++, fs, "Consolas", sellCol);

   SetText("21", "╠════════ ACCOUNT ═════════════╣",             X, Y + step*row++, fs, "Consolas", clrGold);
   SetText("22", "║ Float   " + MoneyText(Profit),               X, Y + step*row++, fs, "Consolas", totalCol);
   SetText("23", "║ Today   " + MoneyText(ProfitToday()),        X, Y + step*row++, fs, "Consolas", normalCol);
   SetText("24", "║ Equity  $" + TestCommaFormat(AccountInfoDouble(ACCOUNT_EQUITY),2),  X, Y + step*row++, fs, "Consolas", normalCol);
   SetText("25", "║ Balance $" + TestCommaFormat(AccountInfoDouble(ACCOUNT_BALANCE),2), X, Y + step*row++, fsTitle, "Consolas", balanceCol);
   SetText("26", "║ MaxDD   $" + TestCommaFormat(MathAbs(MaxDD),2),                      X, Y + step*row++, fs, "Consolas", clrTomato);
   SetText("27", "║ MaxDD%  " + DoubleToString(MathAbs(MaxDDcurr),2) + "%",             X, Y + step*row++, fs, "Consolas", clrTomato);
   SetText("28", "╚══════════════════════════════╝",             X, Y + step*row++, fs, "Consolas", headCol);
}
