...
- `auto`: scope, then record `solo` or `team` before the first artifact change.
...
Terra handles complex, judgment-heavy, architecture-sensitive, or ambiguous bounded work; Luna handles narrow, mechanical, repetitive, extraction, fixture, or documentation-adjacent work. Use `fork_turns="none"`, disjoint write scopes, and a complete [WORKER TASK PACKET](references/contracts.md#worker-task-packet).
